...
export async function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand(`${utils.CMND_NAME}.closeAll`, () => closeAllEditors(true)),
...
function sortList(arr: vscode.Tab[]) {
    return arr.sort((a: vscode.Tab, b: vscode.Tab) => {
        if (a.group.viewColumn > b.group.viewColumn) {
            return 1
        }

        if (b.group.viewColumn > a.group.viewColumn) {
            return -1
        }
